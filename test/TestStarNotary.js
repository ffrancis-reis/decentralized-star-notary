...
it("can add the star name and star symbol properly", async () => {
  ...
  await instance.createStar("Petra Nebula", starId);

  let name = await instance.name();
  let symbol = await instance.symbol();

  assert.equal(name, "Polaris Token");
  assert.equal(symbol, "POL");
});
...
it("lets a user transfer a star", async () => {
  ...
  await instance.createStar("Argos Rho", starId, { from: owner });

  let owner1 = await instance.ownerOf.call(starId);

  await instance.transferStar(accountId, starId);

  let owner2 = await instance.ownerOf.call(starId);

  assert.notEqual(owner1, owner2);
});

it("can lookup star info with token id", async () => {
  // 1. create a Star with different tokenId
  ...
  await instance.createStar("Far Rim", starId);
